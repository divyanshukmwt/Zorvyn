...
import { TrendingUp, TrendingDown, PiggyBank, Zap, Target, Activity } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-sm font-bold font-mono text-slate-200 mt-0.5 truncate">{value}</p>
...
function InsightsPanel() {
  ...
  const insights = useMemo(() => {
    ...
    return { income, expenses, savingsRate, avgTx, topCat, monthChange, currExpenses, prevExpenses };
  }, [transactions]);

  if (!insights) {
    return (
      <div className="bg-card-dark border border-white/[0.07] rounded-card p-6">
        <h2 className="text-sm font-semibold text-slate-200 mb-1">Insights</h2>
        <p className="text-xs text-slate-500 mb-6">Financial health summary</p>
        <p className="text-sm text-slate-600 text-center py-8">Add transactions to see insights</p>
      </div>
    );
  }
  ...
  const rows = [
    ...
    },
    {
      ...
      label: 'Top Category',
      value: topCatMeta
        ? `${topCatMeta.emoji} ${insights.topCat.category}`
        : '—',
      sub: insights.topCat
  ...
  return (
    <div className="bg-card-dark border border-white/[0.07] rounded-card p-6 transition-colors duration-300">
      <div className="mb-5">
        ...
        <p className="text-xs text-slate-500 mt-0.5">Financial health summary</p>
      </div>
