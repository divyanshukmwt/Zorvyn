...
function MobileDrawer({ open, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[200] lg:hidden" style={{ pointerEvents: open ? 'auto' : 'none' }}>
      ...
      <aside
        ref={drawerRef}
        className="absolute left-0 top-0 bottom-0 w-64 bg-surface-dark border-r border-white/[0.07] flex flex-col p-5"
        style={{ transform: 'translateX(-100%)' }}
      >
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-accent-teal to-accent-indigo flex items-center justify-center">
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-lg bg-white/[0.05] text-slate-400 hover:text-slate-200 flex items-center justify-center transition-colors"
          >
            <X size={15} />
          </button>
        </div>
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              ...
              >
                ...
                {item.label}
                {isActive && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-accent-teal" />}
              </button>
            );
          })}
        </nav>
        ...
        <div className="mt-6 pt-5 border-t border-white/[0.06] flex flex-col gap-2">
              ...
              <button
                ...
                className={`
                  ...
                  ${role === r
                    ...
                    : 'bg-transparent border-white/[0.07] text-slate-500'
                  }
                `}
              >
          ...
          <button
            onClick={toggleTheme}
            className="flex items-center justify-center gap-2 py-2.5 rounded-xl bg-bg-dark border border-white/[0.06] text-slate-500 text-xs font-medium"
          >
...
function MobileNav({ onMenuClick }) {
  ...
  return (
    <nav className="
      ...
      bg-surface-dark border-t border-white/[0.07]
      flex pb-safe
    ">
      {NAV_ITEMS.map((item) => {
        const Icon = item.icon;
        ...
        return (
          <button
            ...
            className={`
              ...
              py-3 text-[10px] font-semibold transition-colors
              ${isActive ? 'text-accent-teal' : 'text-slate-600'}
            `}
          >
            <Icon
              size={19}
              strokeWidth={isActive ? 2.25 : 1.75}
              className={isActive ? 'text-accent-teal' : ''}
            />
            {item.label}
          </button>
        );
